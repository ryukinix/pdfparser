...
cdef class Document:
    ...
    property num_of_pages:
        def __get__(self):
            return self._doc.getNumPages()

    cdef void render_page(self, int page_num, OutputDev *dev):
        self._doc.displayPage(dev, page_num, RESOLUTION, RESOLUTION, 0, True, False, False)

    cdef object get_page_size(self, page_num):
            cdef double w,h
            w = self._doc.getPageMediaWidth(page_num)
            h = self._doc.getPageMediaHeight(page_num)
            return (w,h)

    def __iter__(self):
        return self

    def get_page(self, int page):
        return Page(page, self)

    def __next__(self):
        if self._page >= self.num_of_pages:
            raise StopIteration()
        self._page+=1
        return self.get_page(self._page)
...
cdef class Page:
    ...
    property size:
        ...
        def __get__(self):
            return self.doc.get_page_size(self.page_num)
...
cdef class Color:
    ...
    def __richcmp__(x, y, op):
        if isinstance(x, Color) and \
           isinstance(y, Color) and \
           (op == Py_EQ or op == Py_NE):
    ...
    def _get_text(self):
        ...
        while w:
            ...
            for i in range(wlen):
                ...
                # In rare cases font name is not UTF-8 or font name is NULL
                decoded_name = font_name.getCString().decode('UTF-8', 'replace') \
                               if <unsigned long>font_name != 0 else u"unknown"
                last_font = FontInfo(decoded_name,
                                     w.getFontSize(),
                                     Color(r,g,b))
